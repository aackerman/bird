    ...
    **/
    ...
    for(var r in routes) {
      if(routes.hasOwnProperty(r)) {
        //handle twitters insane usage of ids in their routes
        if (typeof routes[r].url != 'string') {
          routes[r].url = routes[r].url[0] + options.id + routes[r].url[1];
          delete options.id;
        }

        if(!routes[r].url.match(/json/)) {
          routes[r].url += options.id + '.json';
          delete options.id;
        }
        //lock in some closure
        this[r] = (function(){
